...
> AI 读：拿到新机器 + 这个项目目录后，按本文档恢复 7 个 MCP server。不要写脚本，逐节执行即可。

## 0. 30 秒判断

```bash
node -v && npm -v && python3 --version && ls .trae/mcp.json
```
...
| weather Python venv | `~/.local/share/weather-mcp/venv/` |
| weather 源码 | `~/.local/share/weather-mcp/weather_mcp_server.py` |
...
### 3.2 装 weather（Python）

```bash
mkdir -p ~/.local/share/weather-mcp && cd ~/.local/share/weather-mcp

# 拉源码（出网失败时加 -x http://127.0.0.1:7897）
curl -sSL -o weather_mcp_server.py \
  https://raw.githubusercontent.com/Zippland/weather-mcp/master/weather_mcp_server.py

# 建 venv 装依赖
python3 -m venv venv
venv/bin/pip install mcp requests pydantic

# 修复源码：FastMCP 在新版 mcp 包（≥1.x）用 instructions= 而不是 description=
# 这一行改动必须做，否则启动直接抛 TypeError
python3 - <<'PY'
import pathlib
p = pathlib.Path("weather_mcp_server.py")
s = p.read_text()
old = '''mcp = FastMCP(
...
```bash
cd ~/.local/share/weather-mcp
OPENWEATHER_API_KEY=<key> ./venv/bin/python weather_mcp_server.py < /dev/null &
sleep 3
pgrep -af weather_mcp_server || echo "FAILED"
pkill -f weather_mcp_server.py
```
...
grep -E "Fail to start|Unauthorized|streamable|TypeError" "$LATEST/window1/exthost/mcp-servers-host.log" | tail -n 30
```

| 症状 | 修复 |
|---|---|
| RollingGo `streamable Unauthorized` | `Authorization` 是 `${env:XXX}` 占位符 → TRAE 不支持，写明文 Key |
| 12306 完全不出现 | `~/.local/share/12306-mcp/` 缺失 → 跑第 3.1 节 |
| 12306 command not found | 路径不对 → 用绝对路径 |
| weather `TypeError: FastMCP.__init__() got an unexpected keyword argument 'description'` | 源码未打补丁 → 跑第 3.2 节 Python 片段 |
| weather 完全不出现 | venv 路径错或源码缺失 → 跑第 3.2 节 |
...
# weather（stdio）
cd ~/.local/share/weather-mcp && OPENWEATHER_API_KEY=<key> ./venv/bin/python -c '
import subprocess, time
p = subprocess.Popen(["./venv/bin/python","weather_mcp_server.py"],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,env={"OPENWEATHER_API_KEY":"<key>","PATH":"/usr/bin:/bin"})
p.stdin.write(b"{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{\"protocolVersion\":\"2025-03-26\",\"capabilities\":{},\"clientInfo\":{\"name\":\"p\",\"version\":\"0\"}}}\n")
p.stdin.flush()
time.sleep(3); print(p.stdout.read().decode(errors="replace")[:300]); p.kill()
'
```
...
2. 跑第 3.1 节装 12306，第 3.2 节装 weather。
3. 跑第 4 节所有端点验证。
4. 告诉用户**完全退出并重启 TRAE**（启动时才读 mcp.json，热重载不全有效）。

## 6. 期望配置

`.trae/mcp.json` 包含 7 个 server，**所有 Key 必须用真实值替换 `<...>` 占位符**：
...
```json
{
  "mcpServers": {
    ...
    "weather":{
      "type": "stdio",
      "command": "/home/lancer/.local/share/weather-mcp/venv/bin/python",
      "args": ["/home/lancer/.local/share/weather-mcp/weather_mcp_server.py"],
      "env": { "OPENWEATHER_API_KEY": "<OPENWEATHER_APIKEY>" }
    }
  }
}
```
...
| `<OPENWEATHER_APIKEY>` | https://home.openweathermap.org/api_keys |
...
1. **新增 MCP**：
   ...
   - 在第 7 节工具名表追加
   - 在第 4 节端点验证追加探针
   - 如需本地二进制/venv 安装步骤，在第 3 节追加
   - 如有兼容性坑或版本要求，在第 9 节追加
2. **删除 MCP**：上面所有相关节同步删除
...
7. weather-mcp（Zippland）源码用旧版 mcp SDK → FastMCP(description=...) 在新版（≥1.x）改为 instructions=，装好后必须打补丁（见第 3.2 节）
8. weather-mcp 调用 OpenWeather `/data/2.5/forecast`（非 One Call 4.0）→ 接受城市名返回 5 天 3 小时间隔预报，不是 One Call 4.0 的逐小时预报