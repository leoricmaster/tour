...
**对策**：
- 每个方案独立的 `images/<方案名>/` 子目录
- 每个方案独立的 `image_credits_<方案名>.json`
- HTML 里 `imagePath` 用相对路径指向自己方案的子目录
- 命名格式：`<方案英文名>.html` + `images/<方案英文名>/` + `image_credits_<方案英文名>.json`

## 4. 改图的标准流程（下次再用）

如果将来需要换图（比如某景点图不满意、或新增景点）：

1. **确认是哪个方案**：进入对应方案的 `images/<方案名>/`
2. 在对应子目录里存新图，命名为 `{ID}.jpg`
3. （可选）运行 fetch 脚本查维基拿到作者/协议信息，写进对应方案的 `image_credits_xxx.json`
4. 把图注对应的 `imageCredit` 字段更新到对应 HTML 的 `#data.sights` JSON 块里
5. **肉眼检查新图是否真的对应该景点**（防止关键词歧义）
6. 浏览器刷新即可

## 5. 维基 API 下载图片的标准模板（下次再用）
...
```
脚本会：
1. 读 `images/<方案名>/` 下的所有 jpg
2. base64 编码后写入对应 HTML 的 `#data.sights[*].imageData`
3. HTML 文件变大到 ~2MB

之后整份 HTML 即可独立转发（不需要 images/ 目录）。

**注意**：当前 `build_base64.py` 是按单个方案写的。多方案时**每个方案一份**（如 `build_base64_phuket.py` / `build_base64_phuquoc_hk.py`）。
